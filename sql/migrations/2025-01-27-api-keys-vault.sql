...
EXCEPTION
    WHEN OTHERS THEN
        ...
        RETURN digest('sinapse-api-keys-master-key-change-this-in-production', 'sha256');
END;
...
SELECT store_api_key('APPLE_SERVICE_ID', 'apple', 'com.ghostmonday.sinapse', 'Apple Service ID (Client ID)', 'production');
...
SELECT store_api_key('APPLE_CLIENT_ID', 'apple', 'com.ghostmonday.sinapse', 'Apple Client ID (fallback)', 'production');
...
SELECT store_api_key('AWS_S3_BUCKET', 'aws', 'sinapse-files', 'AWS S3 Bucket Name', 'production');
