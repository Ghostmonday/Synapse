#!/bin/bash
# Final packaging script for Sinapse iOS

set -e

PROJECT_DIR="$(pwd)"
PACKAGE_NAME="Sinapse_iOS_Final_Build"
TEMP_DIR="/tmp/$PACKAGE_NAME"
ZIP_FILE="$PROJECT_DIR/$PACKAGE_NAME.zip"

echo "📦 Packaging Sinapse iOS Final Build..."
...
if [ -d "Sinapse.xcodeproj" ]; then
    ...
    cp -R Sinapse.xcodeproj "$TEMP_DIR/" 2>/dev/null || true
else
...
Sinapse iOS Final Build Package
Generated: $(date)
...
echo ""
echo "✅ Sinapse build is App Store-ready."
