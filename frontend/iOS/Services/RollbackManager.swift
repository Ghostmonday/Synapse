...
class RollbackManager {
    ...
    func createCheckpoint(componentId: String, modification: ViewModification) -> RollbackCheckpoint {
        ...
        Logger(subsystem: "com.sinapse.app", category: "RollbackManager").info("[RollbackManager] Created checkpoint: \(checkpoint.id) for \(componentId)")
    ...
    func rollbackToLastStableState() async {
        guard let lastCheckpoint = checkpoints.last else {
            Logger(subsystem: "com.sinapse.app", category: "RollbackManager").info("[RollbackManager] No checkpoints to rollback")
            return
        }
    ...
    private func rollbackCheckpoint(_ checkpoint: RollbackCheckpoint) async {
        Logger(subsystem: "com.sinapse.app", category: "RollbackManager").info("[RollbackManager] Rolling back checkpoint: \(checkpoint.id)")
